...
result operateCache(const unsigned long long address, Cache *cache) {
  ...
  if(cache_set(address, cache) > pow(2, cache->setBits)){
    return r;
  }
  //increments the global lru_clock
  ...
  if (probe_cache(address, cache)){
    ...
    if (insert_cacheline(address, cache)){
      ...
      r.victim_block_addr = address_to_block(victim_cacheline(address, cache), cache);
      r.insert_block_addr = address_to_block(address, cache);
     
     
      replace_cacheline(r.victim_block_addr,address, cache);
...
unsigned long long address_to_block(const unsigned long long address,
                                const Cache *cache) {
  /* YOUR CODE HERE */
  int block_bits = cache->blockBits;
  int mask = (1U << block_bits) -1;
  return address & (~mask);
}
...
unsigned long long cache_set(const unsigned long long address,
                             const Cache *cache) {
  /* YOUR CODE HERE */
  unsigned long long index_bit = cache->setBits;
  int mask = (1U << index_bit) -1;
  unsigned long long cacheSet = (address>>cache->blockBits) & mask;
  return cacheSet;
...
bool probe_cache(const unsigned long long address, const Cache *cache) {
  ...
  //1. check valid
  //2. compare tag
  Line*line = cache->sets[cache_set(address, cache)].lines;
  for (int i = 0; i < cache->linesPerSet; i++){
    if (line[i].valid && line[i].tag == cache_tag(address, cache)){
      return true;
    }
  }
  return false;
...
void hit_cacheline(const unsigned long long address, Cache *cache){
  ...
  Line*line = cache->sets[cache_set(address, cache)].lines;
  for (int i = 0; i < cache->linesPerSet; i++){
    if (line[i].tag == cache_tag(address, cache)){
      //update LFU
      line[i].access_counter ++;
      //udate LRU
      line[i].lru_clock = cache->sets[cache_set(address, cache)].lru_clock;
      return;
    }
  }
...
bool insert_cacheline(const unsigned long long address, Cache *cache) {
  ...
  for (int i = 0; i < cache->linesPerSet; i++){
    ...
    if (!line[i].valid){
      //1)
      line[i].tag = cache_tag(address, cache);
      line[i].block_addr = address_to_block(address, cache);
      line[i].valid = 1;
      //2)
      //lru
      line[i].lru_clock = cache->sets[cache_set(address, cache)].lru_clock;
      //initiates lfu
      line[i].access_counter = 0;
      //3)
      return true;
    }
  }
...
unsigned long long victim_cacheline(const unsigned long long address,
  ...
  if (cache->lfu){
    //initialize
    counter = line[0].access_counter;
    clk = line[0].lru_clock;
    adr = line[0].block_addr;
    for (int i = 0; i < cache->linesPerSet; i++){
      //find base on lfu
      if (line[i].access_counter < counter){
          counter = line[i].access_counter;
          clk = line[i].lru_clock;
          adr = line[i].block_addr;
      }
      // when lfu ties, compare lru
      else if (line[i].access_counter == counter){
        if (clk > line[i].lru_clock){
          counter = line[i].access_counter;
          clk = line[i].lru_clock;
          adr = line[i].block_addr;
}
...
void replace_cacheline(const unsigned long long victim_block_addr,
  ...
  /* YOUR CODE HERE */
  Line*line = cache->sets[cache_set(victim_block_addr, cache)].lines;
  //unsigbed long long tag;
  for (int i = 0; i < cache->linesPerSet; i++){
    if (line[i].block_addr == victim_block_addr){
      //change address, tag, lfu counter, lru clk, valid status
      line[i].block_addr = address_to_block(insert_addr, cache);
      line[i].tag = cache_tag(insert_addr, cache);
      line[i].access_counter = 0;
      line[i].lru_clock = cache->sets[cache_set(victim_block_addr, cache)].lru_clock;
      line[i].valid = 1;
...
void cacheSetUp(Cache *cache, char *name) {
  /* YOUR CODE HERE */
  //size = 2^bits
  unsigned long long setSize = pow (2, cache->setBits);
  cache->name = strdup(name);
  //allocate spaces for arr sets
  cache->sets = (Set*)calloc(setSize, sizeof(Set));

  for (int i = 0; i < setSize; i++){
    //allocate spaces for each line
